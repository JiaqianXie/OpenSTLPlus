...
class MambaVP_Model(nn.Module):
    def __init__(
        ...
        self.clip_pos_embed = get_sinusoid_encoding_table(
            num_patches * num_frames // kernel_size,
            clip_decoder_embed_dim
        )
        ...
        samplings = sampling_generator(num_dec_layers, reverse=True)

        self.decoder = nn.Sequential(
            *[ConvSC(dec_dim, dec_dim, spatio_kernel, upsampling=s,
                     act_inplace=act_inplace) for s in samplings[:-1]],
              ConvSC(dec_dim, dec_dim, spatio_kernel, upsampling=samplings[-1],
                     act_inplace=act_inplace)
        )
        self.readout = nn.Conv2d(dec_dim, channels, 1)
    ...
    def forward_features(self, x, mask=None):
        ...
        # mask
        if mask is not None:
            x_vis = x[~mask]  # ~mask means visible
        else:
            x_vis = x
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx == 0:
                skip = self.norm(residual.to(dtype=self.norm.weight.dtype))
        ...
        return x_clip_vis, skip, B, T, W, H

    def forward(self, x, mask=None):
        x_clip_vis, skip, B, T, new_W, new_H = self.forward_features(x, mask)

        # align CLIP
        K, B, _, C_CLIP = x_clip_vis.shape
        expand_clip_pos_embed = self.clip_pos_embed.repeat(B, 1, 1).type_as(x).to(x.device).clone().detach()
        if mask is not None:
            expand_clip_pos_embed = expand_clip_pos_embed[~mask]  # ~mask means visible
        ...
        x_clip_full = rearrange(x_clip_full, 'k b (t n) m -> k (b t) n m', b=B, t=T)
        skip = rearrange(skip, 'b (t n) m -> (b t) n m', b=B, t=T)
        # x_clip = []
        ...
        x_clip_full = x_clip_full.permute(0, 1, 3, 2)
        x_clip_full = x_clip_full.reshape(x_clip_full.size(0), x_clip_full.size(1), x_clip_full.size(2), new_W, new_H)

        x_clip_input = x_clip_full[0]

        skip = skip.permute(0, 2, 1)
        skip = skip.reshape(skip.size(0), skip.size(1), new_W, new_H)
        for i in range(0, len(self.decoder)-1):
            x_clip_input = self.decoder[i](x_clip_input)
        # Y = self.decoder[-1](x_clip_input + skip)
        Y = self.decoder[-1](x_clip_input)
        Y = self.readout(Y)
        Y = Y.reshape(B, T, self.channels, self.img_size, self.img_size)
